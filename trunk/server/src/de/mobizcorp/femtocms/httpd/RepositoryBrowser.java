...
public class RepositoryBrowser extends BasicResource {
    ...
    public RepositoryBrowser(Context ctx) throws IOException {
        ...
        this.source = new HuiSource(new StreamSource(hui.openStream(), hui
                .toString()));
    ...
    protected void process(Request request, Response response)
        ...
        if (url.lastIndexOf('.') != -1) {
            final String name;
            if (slash != -1) {
                name = url.part(slash + 1, url.size() - slash - 1).toString();
            } else {
                name = url.toString();
            }
            InputStream file = HuiSource.getResource(name).openStream();
            if (file != null) {
                final OutputStream out = response.getPrintStream(8192);
                try {
            ...
            while (i.hasMoreElements()) {
                ...
                if (path != null) {
                    ...
                    for (int j = 0; j < values.length; j++) {
                        path.getLast().post(Text.valueOf(values[j]), handler,
                                path);
                    }
                }
            }
